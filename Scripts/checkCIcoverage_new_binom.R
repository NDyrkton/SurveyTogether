...
run.simulation <- function(cl,data.const, data.linear, data.walk, NN = 500, ti = 5){
  ...
  for(j in 1:NN){
    ...
    const.const <- jags.parfit(cl, data.const.phi, c("positiverate"), mod.const.phi,
                               n.chains = 8,n.adapt = 20000,thin = 5, n.iter = 70000,inits = chains.init)
    ...
    #linear model
    linear.linear <- jags.parfit(cl, data.linear.phi, c("positiverate"), mod.linear.phi,
                                 n.chains = 8,n.adapt = 20000,thin = 5, n.iter = 70000,inits = chains.init)
    ...
    #only using unbiased survey (K = 1)
